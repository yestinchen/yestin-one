 ...
 * @package WordPress - Yestin Framework
...
//阅读更多
/*
function yestin_excerpt_more($more) {
	global $post;
	return " <a href=\"". get_permalink($post->ID) . "\">阅读更多</a>";
}*/

//每篇文章的封面图片的获取，加的时候从meta信息中加绝对地址，图片应为正方形
function get_post_cover_image($postID){
	$image_key = '_post_cover_image';
	$image = get_post_meta($postID,$image_key,true);
	if($image==""){
		$image = get_post_meta(1,'_post_cover_image_default',true);
	}
	return $image;
}
